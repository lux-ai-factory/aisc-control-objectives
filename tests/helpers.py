...
# Literal spans of the MCAS graph (tests/fixtures/mcas.ontology.jsonld), from
# the properties the three questions are actually answered from.
CREDIT = "Evaluates creditworthiness"
DEMOGRAPHIC = "bank customers aged 18+"
CHATBOT = "Hosted explanation LLM"


def good_profile() -> Profile:
    """A profile whose three quotes are all genuine spans of the MCAS graph."""
    return Profile(
        high_risk=HighRiskFact(value="yes", quote=CREDIT, source="hasPurpose", annex_iii_point="5(b)"),
        personal_data=Fact(value="yes", quote=DEMOGRAPHIC, source="hasAIUser"),
        interacts_with_natural_persons=Fact(value="yes", quote=CHATBOT, source="hasComponent"),
    )
