...
**The model is chosen in a TOML config file** (`control-objectives.toml`, override path with
`CONTROL_OBJECTIVES_CONFIG_FILE`); **API keys stay in `.env`** (LiteLLM reads the one matching
the model's provider). Precedence: defaults < `control-objectives.toml` < `CONTROL_OBJECTIVES_*` env <
...
- **`control-objectives.toml`** (committed default) + **`.env.example`** document the model
  strings and their keys.

To run on e.g. OpenAI: set `model = "openai/gpt-4o-mini"` in `control-objectives.toml` and
...
- **`control objectives/dimensions.py`** — registry of the 11 dimensions (label/order/
...
- **`config.py`** — `FloorConfig` (`enabled` toggle only; env `CONTROL_OBJECTIVES_FLOOR_ENABLED`).
  The high-risk sector *list* is domain knowledge in the document base
  (`knowledge/high-risk-sectors.md`), loaded by `control objectives/knowledge.py` and injected
...
- **`skills.py`** + `skills/` — `SkillsLoader` (md + YAML frontmatter, routed by
  ...
  point `CONTROL_OBJECTIVES_SKILLS_DIR` at a folder to enable.
- **`rendering.py`** + `templates/` — `render_plan_html` / `render_plan_pdf`.
- **Catalogue `ControlObjectives.tsx`** — dimension-grouped results + **Download PDF**.
...
- **High-risk sectors (D2)** — moved out of config into the **document base**
  ...
  compliance reviewer. `control objectives/knowledge.py` loads it; `control-objectives.toml` keeps only the
...
```
catalogue /control-objectives page  ──upload system card──▶  POST :8090/api/plans/from-card
   └─ frontend/src/pages/ControlObjectives.tsx              └─ aisc_control_objectives.server (composition root)
                                                         ├─ clients/catalogue_http.py  (live catalogue :8001 → tests + checklists)
                                                         ├─ agents/runner.PlanRunner  (prefilter → propose → review)
                                                         └─ anthropic.Anthropic()  (claude-opus-4-8)
```

Built this session (all **additive** — 139 offline tests still green):

- `src/control-objectives/clients/catalogue_http.py` — loads `GET :8001/tool/?detailed=true`,
  splits into `CatalogueTool` (tests) vs `ChecklistDoc` (controls) by the `type`
  tag. Verified live: 20 tests, 17 checklists.
- `src/control-objectives/api/app.py` — added `POST /api/plans/from-card` (accepts an
  uploaded card, no qualification lookup) + CORS.
- `src/control-objectives/server.py` — composition root + uvicorn entry on `:8090`. Reads
  `ANTHROPIC_API_KEY` from env or `control objectives/.env`.
- `pyproject.toml` + `.venv` — added/installed `anthropic`, `httpx`, `uvicorn`.
- Catalogue frontend: `ControlObjectives.tsx` (upload → run → render plan), bundled
  `frontend/public/sample-system-card.json`, `/control-objectives` route, and the grayed
...
1. **Place the API key** (one line, git-ignored):
   ```
   printf 'ANTHROPIC_API_KEY=sk-ant-...\n' > /home/listuser/control-objectives/.env
   ```
2. **Start the service** (or ask Claude to):
   ```
   cd /home/listuser/control-objectives && PYTHONPATH=src .venv/bin/python -m aisc_control_objectives.server
   ```
3. **Test it:** open the catalogue `/control-objectives` page → "Try a sample card" → confirm
...
4. **Decisions to make** (these gate the next build phase):
   ...
   - **Persistence** — plans are in-memory only today. Need the `control_objectives` Postgres
   ...
   - **Reviewer model / lenses** — default is single-reviewer on `claude-opus-4-8`.
     Want the multi-lens panel on by default? (`CONTROL_OBJECTIVES_REVIEW_LENSES=relevance,coverage,parsimony`.)
...
- `PlanRunner` loads catalogue data **once at startup** — restart the
  control objectives after catalogue changes, or add a refresh endpoint.
...
2. **Persistence** — replace `InMemoryPlanStore` with the `control_objectives` Postgres DB so
...
6. **Live integration test** (`CONTROL_OBJECTIVES_LIVE_TESTS=1`) — full MCAS run asserting
   structural invariants (every open_issue covered or gapped; all ids resolvable).
7. Caddy `/control-objectives` route + homepage link-up (cosmetic; deferred in SPEC §0).
...
`config.py` (knobs). Frontend: `catalogue2/frontend/src/pages/ControlObjectives.tsx`.
