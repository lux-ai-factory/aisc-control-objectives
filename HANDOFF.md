...
```
catalogue /control-objectives page ──upload card──▶ POST :8090/api/plans/from-card ──▶ LiteLLM ──▶ provider
  (ControlObjectives.tsx)                         (aisc_control_objectives.server, FastAPI)         (any model)
...
this session; review and commit when ready (both the `aisc-control-objectives` repo and the
catalogue's `ControlObjectives.tsx` / `Catalogue.tsx` / `CatalogueSidebar.tsx` / new
`services/plans.ts`).
...
### Control Objectives service (`/home/listuser/control-objectives`)
...
3. **High-risk sectors → document base** (`knowledge/high-risk-sectors.md`) — the
   ...
   `control-objectives.toml [floor]` keeps only the `enabled` toggle.
4. **Multi-provider via LiteLLM (unified)** — there is **no** native-Anthropic
   ...
   Model is chosen in `control-objectives.toml`; API keys live in `.env` (LiteLLM reads the
...
7. **Result persistence + run history** — `services/plans.ts` (shared store)
   persists the selected plan + a history list in `localStorage`; survives refresh
   **and** control objectives restart. `ControlObjectives.tsx` restores on load and shows "Previous
   runs".
...
# 2. control objectives service (:8090), pointed at the catalogue backend on :8000
cd /home/listuser/control-objectives && source .venv/bin/activate
PYTHONPATH=src CONTROL_OBJECTIVES_CATALOGUE_URL=http://localhost:8000 CONTROL_OBJECTIVES_PORT=8090 python -m aisc_control_objectives.server
```
- Model: `control-objectives.toml` → currently **`openai/gpt-4o`**. Key: `.env` → `OPENAI_API_KEY`.
- Open http://localhost:3000 → Control Objectives → run a card → flip "Recommended" in the catalogue.
- After editing `ControlObjectives/Catalogue/CatalogueSidebar`, rebuild the frontend image:
...
- Tests: `cd /home/listuser/control-objectives && source .venv/bin/activate && python -m pytest -q`
...
- Plans finish `status: "draft"`, not `"reviewed"` — the multi-lens review doesn't
  converge in 3 rounds. Try `max_rounds` ↑ in `control-objectives.toml`, and/or feed the
...
- PDF for an **old** run after a service restart 404s (rendered from server memory).
...
- Persistence is browser-local + control objectives in-memory only. Server-side durable store
  (the planned `control_objectives` Postgres DB) is still deferred — needed for cross-device
